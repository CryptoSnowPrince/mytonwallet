...
export async function buildTokenTransfer(
  ...
  console.log("TG_LOG buildTokenTransfer: ", network, slug, fromAddress, toAddress, amount, data)
  const tokenAddress = await resolveTokenAddress(network, fromAddress, resolveTokenBySlug(slug).minterAddress!);
  console.log("TG_LOG buildTokenTransfer tokenAddress: ", tokenAddress, resolveTokenBySlug(slug).minterAddress!)
  ...
  console.log("TG_LOG buildTokenTransfer tokenAddress: ", tokenWallet, tokenAddress, toNano(TOKEN_TRANSFER_TON_AMOUNT).toString(), payload)
  return {
    tokenWallet,
